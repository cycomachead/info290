...
import requests, re, sys, codecs, os, traceback
...
rdev_matcher = re.compile('^.*\\xa0\\xa0.*$')
        ...
        for link in profile_links:
            if link in error_links:
                ...
                try:
                    ...
                    log("Exception: logging beer %s - %s - %s"%(beer_id,e.message, traceback.format_exc()))
                except Error as e:
                    log("Error: logging beer %s - %s - %s"%(beer_id, e.message, traceback.format_exc()))
                except:
                    log("IDK WTF happened, but beer %s screwed up - %s"%s(beer_id, traceback.format_exc()))
    except Exception as e:
        log("Exception: logging style %s - %s - %s"%(style_dir,e.message, traceback.format_exc()))
    except Error as e:
        log("Error: logging style %s - %s - %s"%(style_dir,e.message, traceback.format_exc()))
    except:
        log("IDK WTF happened, but style %s screwed up - %s"%(style_dir, traceback.format_exc()))
