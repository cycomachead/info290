...
cleanup <- function(D, labeled = TRUE) {
  D <- subset(D, select = -c(home.dest, name, ticket, cabin, embarked)) #, passenger_id))
  #D <- na.omit(D)
  for (i in 1:ncol(D)) {
    if (is.factor(D[,i])) {
...
write.csv(data.frame(passenger_id = test$passenger_id, survived = test.predictions), file = "submissions/test_predictions_alec_rf_only.csv", row.names = FALSE)
