...
train <- read.csv("C:\\Users\\Administrator\\Downloads\\lab5_train.csv")
test <- read.csv("C:\\Users\\Administrator\\Downloads\\lab5_test.csv")
...
q3.rbf.test.zscore.accuracy <- mean((predict(svm.q3.rbf.zscore, newdata = zscore.test) > 0.5) == zscore.test$CAND_ID) #g
...
train1 = train[, -c(8:10)]
test1 = test[, -c(8:10)]
train1[,8]=as.factor(train1[,8])
class(train1[,8])
#lapply(train1, contrasts, contrasts = FALSE)
train2=model.matrix(~ ., data=train1, 
             contrasts.arg = lapply(train1, contrasts, contrasts=FALSE))

q4.net <- nnet(formula = CAND_ID1~., data = train2, size = 10, MaxNWts = 50000)

#below is the soluytion from before
remove.cols <- which(sapply(train, class) == "factor")
train <- cbind(train, model.matrix(~.+0, train))
train <- train1[,-remove.cols]
